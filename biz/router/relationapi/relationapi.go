...
import (
	relationapi "douyin/biz/handler/relationapi"
	...
	root := r.Group("/", rootMw()...)
	{
		_douyin := root.Group("/douyin", _douyinMw()...)
		{
			_relation := _douyin.Group("/relation", _relationMw()...)
			{
				...
				_action.POST("/", append(_changerelationMw(), relationapi.ChangeRelation)...)
			}
			{
				_follow := _relation.Group("/follow", _followMw()...)
				{
					_list := _follow.Group("/list", _listMw()...)
					_list.GET("/", append(_getfollowinglistMw(), relationapi.GetFollowingList)...)
				}
			}
			{
				_follower := _relation.Group("/follower", _followerMw()...)
				{
					...
					_list0.GET("/", append(_getfollowerlistMw(), relationapi.GetFollowerList)...)
				}
			}
			{
				_friend := _relation.Group("/friend", _friendMw()...)
				{
					...
					_list1.GET("/", append(_getfriendlistMw(), relationapi.GetFriendList)...)
				}
			}
		}
	}
